...
CSelfPass::CSelfPass() : _enemy_num(-1),
    _distanceToEnemy(-99999),
    _velAngleToEnemy(-99999),
    _velDiffToEnemy(-99999),
    _selfpassStatus("SelfPass: the nearest enemy is "),
...
void CSelfPass::draw_selfpassMsg(const CVisionModule *pVision, const int leader){
    ...
    GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(-580, 260), (QString("Distance to nearest enemy : %1").arg(QString::number(_distanceToEnemy))).toLatin1(), COLOR_GREEN);
    GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(-580, 280), (QString("Angel vel to nearest enemy : %1").arg(QString::number(_velAngleToEnemy))).toLatin1(), COLOR_GREEN);
    GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(-580, 300), (QString("Diff vel to nearest enemy : %1").arg(QString::number(_velDiffToEnemy))).toLatin1(), COLOR_GREEN);
    GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(-580, 320), _selfpassCannot.toLatin1(), COLOR_GREEN);
...
void CSelfPass::cal_variousPoint(const CVisionModule *pVision, const int leader){
    ...
    cal_angelVelToMove(pVision, leader);
}

//计算leader距离敌方最远的点_farEnemyPoint
...
//计算leader当前运动方向的点_moveDirectPoint
...
void CSelfPass::cal_angelVelToMove(const CVisionModule *pVision, const int leader){
    const PlayerVisionT& me = pVision->OurPlayer(leader+1);
    ...
    _velAngleToEnemy = (1.0/400) * (x1 * x2 + y1 * y2) / sqrt(x1 * x1 + y1 * y1) * sqrt(x2 * x2 + y2 * y2);
}
...
bool CSelfPass::is_angelOriToEnemy(const CVisionModule *pVision, const int leader, double xita){
...
bool CSelfPass::is_angelOriToMove(const CVisionModule *pVision, const int leader, double xita){
    ...
    if(sin(xita/2) > dis_h / dis_r){return true;qDebug()<<"is_anglenough_MeMove";}
    else{return false;}
}

//判断相对速度方向与_moveEnemyPoint夹角大于xita
bool CSelfPass::is_angelVelToMove(const CVisionModule *pVision, const int leader, double xita){
    if(_moveDirectPoint.x() < -9999 || _moveDirectPoint.y() < -9999){return false;}
    ...
    if(cos(xita) > cos_angle){return true;qDebug()<<"is_anglenough_EnemyVel";}
    else{return false;}
}
...
bool CSelfPass::is_selfpassPoint(const CVisionModule *pVision, const int leader){
    if(is_angelOriToEnemy(pVision, leader, 0.523)
            && is_angelOriToMove(pVision, leader, 0.261)
            /*&& (is_angelpenaltyPoint(pVision, leader, 450))*/
            /*&& is_angelVelToMove(pVision, leader, 2.61)*/
...
//计算传球时各种参数input
double CSelfPass::input_angelOriToMove(const CVisionModule *pVision, const int leader){
    if(_moveDirectPoint.x() < -9999 || _moveDirectPoint.y() < -9999){return -99999;}
    if(_orientatePoint.x() < -9999 || _orientatePoint.y() < -9999){return -99999;}
    const PlayerVisionT& me = pVision->OurPlayer(leader+1);
    double dis_h = 0.5 * (_moveDirectPoint.dist(_orientatePoint));
    double dis_r = _moveDirectPoint.dist(me.Pos());
    return (dis_h / dis_r);
...
double CSelfPass::input_angelOriToEnemy(const CVisionModule *pVision, const int leader){
    if(_farEnemyPoint.x() < -9999 || _farEnemyPoint.y() < -9999){return -99999;}
    if(_orientatePoint.x() < -9999 || _orientatePoint.y() < -9999){return -99999;}
    const PlayerVisionT& me = pVision->OurPlayer(leader+1);
    double dis_h = 0.5 * (_farEnemyPoint.dist(_orientatePoint));
    double dis_r = _farEnemyPoint.dist(me.Pos());
...
double CSelfPass::input_angelVelToMove(const CVisionModule *pVision, const int leader){
    if(_moveDirectPoint.x() < -9999 || _moveDirectPoint.y() < -9999){return -99999;}
    if(_distanceToEnemy < -9999){return -99999;}
    const PlayerVisionT& me = pVision->OurPlayer(leader+1);
    double x1 = temp_enemyVel.x() - me.Pos().x();
    double y1 = temp_enemyVel.y() - me.Pos().y();
    double x2 = _moveDirectPoint.x() - me.Pos().x();
...
//需要判定传球
void CSelfPass::output_saveData(const CVisionModule *pVision, const int leader){
    FILE *fp = fopen("../../data_selfpass.txt", "a");
    fprintf(fp, "%f\n", input_angelOriToMove(pVision, leader));
    fprintf(fp, "%f\n", input_angelOriToEnemy(pVision, leader));
    fprintf(fp, "%f\n", input_angelVelToMove(pVision, leader));
    fclose(fp);
}

void CSelfPass::output_selfPass(const CVisionModule *pVision, const int leader){
