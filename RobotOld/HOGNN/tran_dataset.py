...
embed_dim = 128
## pandas中DataFrame的各种函数
# lambda x                          表示一个任意变量x
# df.map(lambda x，x+1)             对df中的每一个元素进行函数运算x+1
# df.groupby("标签")                根据标签对df分组，标签相同的数据放到一起
# df.groupby("标签").agg('mean')    计算df分组后其他标签数据的平均值
# df.head()                     查看前几行df数据
# df.loc["行标签","列标签"]      根据行和列的标签选取数据，取出某一行列的数据
# df.iloc[行,列]                根据行和列的序号选取数据，取出某一行列的数据
# df.drop("标签")               根据行的标签删除该行，删除列要加个axis=1
# df.unique()           返回df数据中不同的值
# df.nunique()          返回df数据中不同值的个数
# df.squeeze()          压缩，减去df一个维度
# df.unsqueeze()        扩充，增加df一个维度
# df.transpose(a,b)     转置，高维数据的矩阵转置，交换a和b号坐标轴
# df.choice(df)         抽卡，返回df的随机一项
# df.isin('a')          过滤，判断df中是否出现字符串a
