...
["shoot"] = {
	...
	Assister = BACK_PASS and task.zget(_, _, _, flag.kick) or task.goSpeciPos(WAIT_POS3, _, flag.allow_dss + flag.free_kick + flag.not_avoid_our_vehicle),
	Leader   = task.goSpeciPos(FAKE_POS3, _, DSS_FLAG),
        Breaker  = BACK_PASS and task.goSpeciPos(WAIT_POS2, _, flag.allow_dss + flag.free_kick + flag.not_avoid_our_vehicle) or task.zget(_, _, _, flag.kick),
	Special  = task.goSpeciPos(FAKE_POS2, _, DSS_FLAG),
	Middle   = task.goSpeciPos(Middle_POS1, _,   DSS_FLAG),
	Center   = task.leftBack(),
	Defender = task.rightBack(),
