...
local PASS_POS1 = CGeoPoint:new_local(450*0.5, -150*0.5)
local WAIT_POS2 = ball.refAntiYPos(CGeoPoint:new_local(250*0.5, 40*0.5))
local WAIT_POS3 = ball.refSyntYPos(CGeoPoint:new_local(220*0.5, 200*0.5))
...
local PASS_POS2_TMP = ball.refSyntYPos(CGeoPoint:new_local(100*0.5, -320*0.5))

local FAKE_POS2 = ball.refSyntYPos(CGeoPoint:new_local(560*0.5, 200*0.5))
local FAKE_POS3 = ball.refSyntYPos(CGeoPoint:new_local(550*0.5, 140*0.5))
local Middle_POS1 = ball.refSyntYPos(CGeoPoint:new_local(350*0.5, 120*0.5))
local Middle_POS2 = ball.refSyntYPos(CGeoPoint:new_local(440*0.5, -320*0.5))
local Center_POS1 = ball.refSyntYPos(CGeoPoint:new_local(380*0.5, -40*0.5))
local Center_POS2 = ball.refSyntYPos(CGeoPoint:new_local(500*0.5, -170*0.5))
...
["pass1"] = {
	...
	Assister = task.zget(WAIT_POS1,_,_,flag.kick+flag.chip),--task.chipPass(PASS_POS1),
	Leader   = task.goSpeciPos(WAIT_POS1, player.toPlayerDir("Assister"),  DSS_FLAG),
...
["pass2"] = {
	...
	Assister = task.goSpeciPos(WAIT_POS3, _, DSS_FLAG),
	Leader   = task.InterTouch(_,PASS_POS2),
...
["shoot"] = {
	...
	Assister = BACK_PASS and task.InterTouch() or task.goSpeciPos(WAIT_POS3, _, flag.allow_dss + flag.free_kick + flag.not_avoid_our_vehicle),
	Leader   = task.goSpeciPos(FAKE_POS3, _, DSS_FLAG),
        Breaker  = BACK_PASS and task.goSpeciPos(WAIT_POS2, _, flag.allow_dss + flag.free_kick + flag.not_avoid_our_vehicle) or task.InterTouch(),
	Special  = task.goSpeciPos(FAKE_POS2, _, DSS_FLAG),
	Middle   = task.goSpeciPos(Middle_POS1, _,   DSS_FLAG),
	Center   = task.leftBack(),
	Defender = task.rightBack(),
