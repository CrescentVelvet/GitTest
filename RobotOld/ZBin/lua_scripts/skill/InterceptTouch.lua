function InterceptTouch(task)
  ...
  execute = function(runner)
    ...
    --add by Wang in 2018/06/18
    if type(task.target) == "function" then
      mtarget = task.target(runner)
    else
      mtarget = task.target
    end
    ...
    return CInterceptTouch(runner, mpos:x(), mpos:y(), mdir, mpower, mbuffer, useChip, testMode, mtarget:x(), mtarget:y())
  end
