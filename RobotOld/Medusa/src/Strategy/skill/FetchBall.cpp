...
void CFetchBall::plan(const CVisionModule* pVision) {
    ...
    if(ball.Valid() && Utils::OutOfField(ball.RawPos(), 2*Param::Vehicle::V2::PLAYER_SIZE)){//ball out of field
        if(fraredOn < 2*BPFraredOn){//not have ball
            ...
            setSubTask(PlayerRole::makeItGoto(vecNumber, Utils::MakeInField(me.RawPos(), 50), me.RawDir(), CVector(0, 0), 0, 300, 5, 300, 5, PlayerStatus::ALLOW_DSS|PlayerStatus::NOT_AVOID_PENALTY));
        }
    }
    else{
        if(fraredOn < 2*BPFraredOn && cnt == 0){//not have ball
            ...
            if(!SAFEMODE){
                ...
                setSubTask(PlayerRole::makeItGoto(vecNumber, me.RawPos()+Utils::Polar2Vector(30, me2target.dir()), me2target.dir(), CVector(0, 0), 0, 300, 50, 300, 50, PlayerStatus::ALLOW_DSS|PlayerStatus::NOT_AVOID_PENALTY));
...
//                        setSubTask(PlayerRole::makeItGoto(vecNumber, me.RawPos(), me.RawDir(), CVector(0, 0), 0, 300, 50, 300, 50, PlayerStatus::ALLOW_DSS|PlayerStatus::NOT_AVOID_PENALTY));
                        setSubTask(PlayerRole::makeItGoto(vecNumber, me.RawPos(), me2target.dir(), CVector(0, 0), 0, 200, 20, 200, 10, PlayerStatus::ALLOW_DSS|PlayerStatus::NOT_AVOID_PENALTY));
//                    }
                }
                else{
                    ...
                    setSubTask(PlayerRole::makeItGoto(vecNumber, target + Utils::Polar2Vector(-Param::Vehicle::V2::PLAYER_CENTER_TO_BALL_CENTER, me2target.dir()), me2target.dir(), CVector(0, 0), 0, 200, 20, 200, 10, PlayerStatus::ALLOW_DSS|PlayerStatus::NOT_AVOID_PENALTY));
                }
            }
        }
    }
