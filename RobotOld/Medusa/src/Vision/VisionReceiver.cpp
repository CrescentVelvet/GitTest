        ...
        while (visionSocket->state() == QUdpSocket::BoundState && visionSocket->hasPendingDatagrams()) {
            ...
            if (ballFound) {
                ...
                if (ball.x() < -30000 && ball.y() < -30000) {
                    ...
                    temp.rawBall.x = ball.raw_x() / 10;
                    temp.rawBall.y = - ball.raw_y() / 10;
                    temp.chipPredict.x =  ball.chip_predict_x() / 10;
                    temp.chipPredict.y = -ball.chip_predict_y() / 10;
                    temp.ball.valid = ball.valid();
            ...
            for (i = 0; i < robots_blue_n; i++) {
                ...
                if (id >= 0 && id < Param::Field::MAX_PLAYER_NUM) {
                    ...
                    temp.player[index].rawPos.x = robot.raw_x() / 10;
                    temp.player[index].rawPos.y = -robot.raw_y() / 10;
                    temp.player[index].pos.valid = robot.valid();
                    temp.player[index].dir = -robot.orientation();
                    temp.player[index].rawdir = - robot.raw_orientation();
                    temp.player[index].vel.setVector(robot.vel_x() / 10, -robot.vel_y() / 10);
                    temp.player[index].raw_vel.setVector(robot.raw_vel_x() / 10, -robot.raw_vel_y() / 10);
                    temp.player[index].dirvel = -robot.rotate_vel();
                    temp.player[index].raw_dirVel = -robot.raw_rotate_vel();
                    temp.player[index].accelerate.setVector(robot.accelerate_x() / 10, -robot.accelerate_y() / 10);
                }
            }
            //Yellow Car
...
ZSS_THREAD_FUNCTION void VisionReceiver::getMessage() {
    ...
    while (true) {
        ...
        temp.next_command = _referee->getNextCommand();
