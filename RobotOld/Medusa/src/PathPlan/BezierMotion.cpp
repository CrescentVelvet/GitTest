...
namespace {
    ...
    const double upperBoundSpeedLimitRotate = 200;
    const double lowerBoundCurvaLimitRotate = 0.00;
    const double upperBoundCurvaLimitRotate = 0.02;
...
void BezierMotion::bezierMotion1d(double x0, double v0, double v1, double aMax, double dMax, double vMax, double flatVel, double &trajAccel, double &trajTime, double &trajTimeAcc, double &trajTimeDec, double &trajTimeFlat) {
    if(x0 == 0 && v0 == v1) {
    ...
    if(v0 * x0 < 0 || (fabs(v0) > fabs(v1) && decelDistToV1 > fabs(x0))) {
        ...
        bezierMotion1d(x0 + copysign(distToStop, v0), 0, v1, aMaxOri, dMaxOri, vMax, flatVel, trajAccel, trajTime, trajTimeAcc, trajTimeDec, trajTimeFlat);
...
void BezierMotion::bezierMotion2d(const stateNew &start, const stateNew &target, const PlayerCapabilityT &capability, const double flatVelFactor,CVector &trajAccel, double &time) {
    CVector v0 = start.vel;
    CVector v1 = target.vel;
    CVector x0 = target.pos - start.pos;
    double vMax = capability.maxSpeed;
    double flatVel = capability.maxSpeed * flatVelFactor;
    ...
    bezierMotion1d(x0.x(), v0.x(), v1.x(), aMax, dMax, vMax, flatVel, trajAccelX, xTime, xTimeAcc, xTimeDec, xTimeFlat);
    bezierMotion1d(x0.y(), v0.y(), v1.y(), aMax, dMax, vMax, flatVel, trajAccelY, yTime, yTimeAcc, yTimeDec, yTimeFlat);
...
void BezierMotion::bezierGotoPoint(const stateNew &start, const stateNew &target, const PlayerCapabilityT &capability, const double &accelFactor, const double &angleAccFactor, const double flatVelFactor,stateNew &nextFrame, double nextCurva) {
    CVector trajAcel;
    double time;
    ...
    isMovePlanner = true;
    bezierMotion2d(start, target, capability, flatVelFactor, trajAcel, time);
//    compute_motion_2d(start.pos - target.pos, start.vel, target.vel, capability.maxAccel,capability.maxDec, capability.maxSpeed, accelFactor, trajAcel, time);
    ...
    double rotateFactorSpeed, rotateFactorCurva;
    double selfSpeed = start.vel.mod();
    if(upperBoundSpeedLimitRotate == lowerBoundSpeedLimitRotate)
        cout << "Oh shit!!! Error speed limit params!!! ---BezierMotion.cpp" << endl;
    else if(selfSpeed < lowerBoundSpeedLimitRotate)
        rotateFactorSpeed = 1.0;
    else if(selfSpeed < upperBoundSpeedLimitRotate)
        rotateFactorSpeed = 1.0 - (selfSpeed - lowerBoundSpeedLimitRotate) / (upperBoundSpeedLimitRotate - lowerBoundSpeedLimitRotate);
    else
        rotateFactorSpeed = 0.0;

    if(lowerBoundCurvaLimitRotate == upperBoundCurvaLimitRotate)
        cout << "Oh shit!!! Error curvature limit params!!! ---BezierMotion.cpp" << endl;
    else if(nextCurva < lowerBoundCurvaLimitRotate)
        rotateFactorCurva = 1.0;
    else if(nextCurva < upperBoundCurvaLimitRotate)
        rotateFactorCurva = 1.0 - (nextCurva - lowerBoundCurvaLimitRotate) / (upperBoundCurvaLimitRotate - lowerBoundCurvaLimitRotate);
    else
        rotateFactorCurva = 0.0;

    if(capability.maxAngularAccel < minMaxAngleAcc)
        maxAngleAccel= maxAngleDec = capability.maxAngularAccel;
    else
        maxAngleAccel = maxAngleDec = minMaxAngleAcc + (capability.maxAngularAccel * rotateScaleFactor - minMaxAngleAcc) * rotateFactorSpeed * rotateFactorCurva;

    if(fabs(deltaAngle) > minAngleLimitRotateSpeed) {
        if(capability.maxAngularSpeed < minMaxAngleSpeed)
            maxAngleVel = capability.maxAngularSpeed;
        else
            maxAngleVel = minMaxAngleSpeed + (capability.maxAngularSpeed * rotateScaleFactor - minMaxAngleSpeed) * rotateFactorSpeed * rotateFactorCurva;
    }

    compute_motion_1d(-deltaAngle, angleVel0, angleVel1, maxAngleAccel, maxAngleDec, maxAngleVel, angleAccFactor, 1.0, rotateAccel, timeRotate, timeRotateAccel, timeRotateDec, timeRotateFlat, ROTATE);
