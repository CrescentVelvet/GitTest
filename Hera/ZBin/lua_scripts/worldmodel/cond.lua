...
function  canExitMRLFrontDef()
	local result = false
	if ball.velMod()<150 * 10 and vision:ballVelValid() then
		result = true
	--else
...
function canBeImmortalShoot()
	local result = false
	if ball.toPointDist(enemy.bestPos()) < 50 * 10 and vision:ballVelValid() then
		result = true
	end
	return result
end
...
function  checkBallPassed(p1,p2)
	...
	if ball.velMod()> 50 * 10 and ball.toBestEnemyDist()> 15 * 10 and flag == false then
		check = enemy.bestDir()
	...
	if flag == true then
		...
		local checkLine = CGeoLine:new_local(CGeoPoint:new_local(checkPosX,checkPosY),CGeoPoint:new_local(checkPosX,checkPosY)+Utils.Polar2Vector(100 * 10,check))
