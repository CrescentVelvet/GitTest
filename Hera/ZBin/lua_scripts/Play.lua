...
--~		1 ---> task, 2 --> matchpos, 3--->kick, 4 --->dir, 5 --->pre, 6 --->kp, 7--->cp, 8--->flag
		kickStatus:clearAll()
		for rolename, task in pairs(curPlay[gRealState]) do
			if (type(task) == "function" and rolename ~= "match" and (gRoleNum[rolename] ~= nil or type(rolename)=="function" or type(rolename)=="number")) then
				task = task(gRoleNum[rolename])
			end
			if (type(task) == "table" and rolename ~= "match" and (gRoleNum[rolename] ~= nil or type(rolename)=="function" or type(rolename)=="number")) then
--~ 			统一进行射门和吸球的管理,并做任务处理
				if task[1] == nil then
				...
				elseif type(rolename) == "number" then
					roleNum = rolename
				else
					print("Error!! in Play.lua line:225, rolename type error!")
				end
				...
				if roleNum ~= -1 then
					if task[3] ~= nil and task[3](roleNum) ~= kick.none() then
						...
						-- end
						if bit:_and(mflag, flag.dribbling) ~= 0 then
					...
					if type(rolename) == "number" then
						rolename = string.format("%x",rolename)
						debugEngine:gui_debug_msg(vision:ourPlayer(roleNum):Pos(), string.sub(rolename, 1, 1),10,255255000,110,99)
					elseif(type(rolename)=="string") then
						debugEngine:gui_debug_msg(vision:ourPlayer(roleNum):Pos(), string.sub(rolename, 1, 1),10,255255,110,99)
					end
