...
class CMessiDecision {
  public:
    ...
    int attackerNum(int i) {
        if (i < 0) return _attacker.front();
        else if (i >= _attacker.size()) return _attacker.back();
        else return _attacker.at(i);
    }
    string nextState() {
        return _state;
    }
    ...
    bool canDirectKick();
    void reset();
    CGeoPoint _speedMePos;
    CGeoPoint _speedCarPos;
    CGeoPoint _speedBallPos;
private:
    int _leader;
    int _receiver;
    int _cycle;
