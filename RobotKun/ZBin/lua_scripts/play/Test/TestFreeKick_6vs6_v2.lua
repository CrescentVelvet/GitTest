...
local TimeNotUsed = 0
local temp_pos = CGeoPoint:new_local(param.pitchLength / 4 + 500, 0)
...
local function linePos (number)
	return function ()
		-- if math.abs((wait_pos + Utils.Polar2Vector(line_x * (number - 1) , dirFlag * math.pi / 2)):x()) > param.pitchLength / 2 - param.penaltyWidth - 50
		-- and math.abs((wait_pos + Utils.Polar2Vector(line_x * (number - 1) , dirFlag * math.pi / 2)):y()) < param.pitchLength / 2  then
		-- 	return temp_pos + Utils.Polar2Vector(line_x * (number - 1) , dirFlag * math.pi / 2)
	 --  	end

		return wait_pos + Utils.Polar2Vector(line_x * (number - 1) , dirFlag * math.pi / 2) --math.pi / 4 (pass_pos - fake_pos):dir() +
	end
end

local STOP_BALL_PLACE = true
...
local KICKER_TASK = function(pos)
	return function()
		...
		if cond.theirBallPlace() and not (STOP_BALL_PLACE and BP_FINISH) then 
			return task.staticGetBall(pos, _, flag.free_kick + flag.allow_dss)
			-- return task.goCmuRush(CGeoPoint:new_local(-400, -400), player.toBallDir, _, DSS_FLAG)
		end

		debugEngine:gui_debug_msg(ball.pos()+CVector:new_local(30,30),"Static " .. (BP_FINISH and "T" or "F"),4)
		return task.staticGetBall(pos, _, flag.free_kick + flag.allow_dss + flag.our_ball_placement)
	end
end
local LEFTBACK_TASK = function ()
	return function ()
		return BACK_KICK and task.leftBack() or task.goCmuRush(linePos(2), player.toBallDir, _, DSS_FLAG)
	end
end
local RIGHTBACK_TASK = function ()
	return function ()
		return BACK_KICK and task.rightBack() or MIDDLE_KICK and task.singleBack() or task.goCmuRush(linePos(3), player.toBallDir, _, DSS_FLAG)
	end
end
local function freeKickDebug ()
	...
	debugEngine:gui_debug_msg(CGeoPoint:new_local(-400, -500) * (IS_RIGHT and -1 or 1), "ballmoved: "..tostring(ballmove))
end

local function changeChipReceiver()
	local leader = messi:leaderNum()
	if leader ~= gRoleNum["Leader"] then
		gRoleNum["Receiver"] = leader
	end 
end 
...
["GoLine"] = {
	switch = function()
		...
		if STOP_BALL_PLACE and not cond.ourBallPlace() and not cond.theirBallPlace() then
			pass_pos = messi:freeKickPos()
			if pass_pos:y() > 0 then
				dirFlag  = 1
			else
	...
	Leader   = KICKER_TASK(PASS_POS),
	Receiver = task.goCmuRush(linePos(0), player.toBallDir, _, DSS_FLAG),
	-- Receiver = task.staticGetBall(WAIT_POS, _, flag.free_kick + flag.allow_dss),
...
["Judge"] = {
	...
	Receiver = task.goCmuRush(linePos(4), player.toBallDir, _, DSS_FLAG),  -- ??? 
	Assister = task.continue(),
	Special  = LEFTBACK_TASK(),
	Defender = RIGHTBACK_TASK(),
	Goalie   = task.zgoalie(),
...
["DirectKick"] = {
	...
	Leader   = task.zget(_, _, _, flag.kick + flag.safe + flag.norush),
...
["FixPass"] = {
	switch = function()
		...
		if bufcnt(player.kickBall("Leader"), 1, 100) then
			TimeNotUsed, FIX_CNT = cp.toTargetTime(PASS_POS)
			if FIX_CNT <= 0 then
				FIX_CNT = 1
			end
...
["Fix"] = {
	switch = function()
		...
		if skillUtils:isInFlyTime() then
			--changeChipReceiver()
			return "ChipPass"
		end
	end,
...
["ChipPass"] = {
	switch = function()
		freeKickDebug()
		if not skillUtils:isInFlyTime() then
			return "Shoot"
		end
	end,
	Leader   = task.zget(PASS_POS, _, _, flag.kick + flag.chip + flag.dribble + flag.safe),
	Receiver = task.zattack(_, _, _, flag.kick + flag.safe),
	Assister = task.goCmuRush(FAKE_POS, player.toBallDir, _, AFTER_KICK_FLAG),
	Special  = task.leftBack(),
	Defender = task.rightBack(),
	Goalie   = task.zgoalie(),
...
["Shoot"] = {
	...
	Assister = task.zmarking("First"),
	-- Assister = task.goCmuRush(pos.getOtherPos(2), player.toBallDir, _, AFTER_KICK_FLAG),
	Special  = task.leftBack(),
	Defender = task.rightBack(),
	Goalie   = task.zgoalie(),
	match = "{R}[SD][LA]"
