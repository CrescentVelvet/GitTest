...
local function linePos (number)
    return function ()
        if math.abs((wait_pos + Utils.Polar2Vector(line_x * (number - 1) , dirFlag * math.pi / 2)):x()) > param.pitchLength / 2 - param.penaltyWidth 
...
["Judge"] = {
    ...
    Receiver = task.goCmuRush(PASS_POS, player.toBallDir, _, DSS_FLAG),  -- ??? 
    Assister = task.continue(),
    Special  = LEFTBACK_TASK(),
    Defender = RIGHTBACK_TASK(),
    Goalie   = task.zgoalie(),
...
["Chip"] = {
    switch = function()
        freeKickDebug()
        local ballpos = ball.chipSecondPos();
        if not ball.isChip() then
            SecondCount = 0.4 *ball.chipFlyTime()*param.frameRate
            return "ChipSecond"
        end
        debugEngine:gui_debug_line(player.pos("Receiver"), ballpos)
    end,
    Leader   = task.zget(ball.chipSecondPos, _, _, flag.kick + flag.chip + flag.dribble + flag.safe),
    Receiver = task.goCmuRush(ball.chipSecondPos, player.toBallDir),
    Assister = task.goCmuRush(FAKE_POS, player.toBallDir, _, AFTER_KICK_FLAG),
    Middle   = task.goCmuRush(linePos(2), player.toBallDir, _, AFTER_KICK_FLAG),
    Fronter  = task.goCmuRush(linePos(3), player.toBallDir, _, AFTER_KICK_FLAG),
    Special  = task.leftBack(),
...
["ChipSecond"] = {
    switch = function()
        freeKickDebug()
        local ballpos = ball.chipSecondPos();
        if bufcnt(true,SecondCount) then
            return "Shoot"
        end
        debugEngine:gui_debug_line(player.pos("Receiver"), ballpos)
    end,
...
name = "TestFreeKick_use_chip_predict",
applicable ={
    exp = "a",
    a = true
},
