...
function penaltyTurn(d, k)
	...
	local mexe, mpos = SpeedInRobot{speedW = -2.5, speedX = 0} --700 -- 点球甩飞改 speedW
...
function slideToPos(p, d, f)
	...
	local mexe, mpos = SlideGoto{pos = p, dir = idir, flag = iflag}
	return {mexe, mpos}
end
...
function zback(guardNum, index, p, f, defendNum)
	local ipower
	...
	local mexe, mpos = ZBack{ guardNum = guardNum, index = index, power = ipower, flag = iflag , defendNum = defendNum}
	return {mexe, mpos}
end
...
function openSpeed(vx, vy, vdir, f)
	local spdX = vx
	local spdY = vy
	local spdW = vdir
	local mflag = f or 0
	...
	return {mexe, mpos, kick.flat, dir.defendBackClear(), pre.fieldDefender(), kp.specified(0),cp.specified(0), mflag}
end
