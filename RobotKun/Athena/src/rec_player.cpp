...
//        processMsg->set_size(robot_vision.robotSize[color]);
        for(int j = 0; j < PARAM::ROBOTNUM; j++) {
            ...
            result.setRobot(color,
                            ...
                            robot.angle(),
                            robot.valid());
            GlobalData::instance()->robotInformation[color][j].imuDir = robot.imudir();
            GlobalData::instance()->robotInformation[color][j].imucleaned = robot.imucleaned();
            GlobalData::instance()->robotInformation[color][j].battery = robot.battery();
            GlobalData::instance()->robotInformation[color][j].capacitance = robot.capacitance();
            GlobalData::instance()->robotInformation[color][j].infrared = robot.infrared();
        }
    }
...
void RecPlayer::run() {
    ...
    while (_run && ++_currentFrame < packets.size() - 1 && this->isRunning()) {
        sendMessage(packets.at(_currentFrame));
        emit positionChanged(_currentFrame);
        QThread::currentThread()->msleep(6);
        GlobalSettings::instance()->needRepaint();
        QThread::currentThread()->msleep(6);
    }
}
