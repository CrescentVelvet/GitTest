...
double CDealBall::calculateWeight(int camID, CGeoPoint ballPos) {
    ...
    if (ballPos.x() > camera.leftedge.max && ballPos.x() < camera.rightedge.max &&
        ...
        return 1 / (1 + exp(- abs(ballPos.x() - camera.leftedge.min) / abs(camera.leftedge.max - camera.leftedge.min)));
    else if (ballPos.x() > camera.rightedge.max && ballPos.x() < camera.rightedge.min)
        return 1 / (1 + exp(- abs(ballPos.x() - camera.rightedge.min) / abs(camera.rightedge.max - camera.rightedge.min)));
    else if (ballPos.y() < camera.downedge.max && ballPos.y() > camera.downedge.min)
        return 1 / (1 + exp(- abs(ballPos.y() - camera.downedge.min) / abs(camera.downedge.max - camera.downedge.min)));
    else if (ballPos.y() > camera.upedge.max && ballPos.y() < camera.upedge.min)
        return  1 / (1 + exp(- abs(ballPos.y() - camera.upedge.min) / abs(camera.upedge.max - camera.upedge.min)));
...
void CDealBall::mergeBall() {
    ...
    for ( i = 0; i < origin.ballSize; i++) {
        bool found = false;
//        qDebug()<<"origin ballsize: "<<origin.ballSize;
...
//    qDebug() << "Actually have " << actualBallNum << " balls";
...
//    qDebug()<<"ball velocity: "<<ballVel.mod()<<"lost frame: "<<lostFrame;
