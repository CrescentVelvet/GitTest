...
int DIFF_VECHILE_MAX_DIFF = 1000;//1000
float MAX_SPEED = 5000;
// 由于图像的一次事故，帧率和系统延时改变了
const double TOTAL_MOV_LATED_FRAME = 2.0f; //平移的延时(原来为4.2)
...
double CDealRobot::calculateWeight(int camID, CGeoPoint roboPos) {  //change linear function to sigmoid function
    ...
    if (roboPos.x() > camera.leftedge.max && roboPos.x() < camera.rightedge.max &&
        ...
        return  1 / (1 + exp(- abs(roboPos.x() - camera.leftedge.min) / abs(camera.leftedge.max - camera.leftedge.min)));
    else if (roboPos.x() > camera.rightedge.max && roboPos.x() < camera.rightedge.min)
        return 1 / (1 + exp(- abs(roboPos.x() - camera.rightedge.min) / abs(camera.rightedge.max - camera.rightedge.min)));
    else if (roboPos.y() < camera.downedge.max && roboPos.y() > camera.downedge.min)
        return 1 / (1 + exp(- abs(roboPos.y() - camera.downedge.min) / abs(camera.downedge.max - camera.downedge.min)));
    else if (roboPos.y() > camera.upedge.max && roboPos.y() < camera.upedge.min)
        return 1 / (1 + exp(- abs(roboPos.y() - camera.upedge.min) / abs(camera.upedge.max - camera.upedge.min)));
        ...
        for (int roboId = 0; roboId < PARAM::ROBOTNUM; roboId++) {
            ...
            for (int camId = 0; camId < PARAM::CAMERA; camId++) {
                ...
                if(robotSeqence[color][roboId][camId].pos.x() < 88888 && robotSeqence[color][roboId][camId].pos.y() < 88888) {
                    ...
                    teamWeight += _weight;
                    //qDebug()<<"cameraId: "<<camId<<"weight: "<<_weight<<"robotID"<<roboId;
    ...
    for (int team = 0; team < PARAM::TEAMS; team ++) {
        ...
        //   Robot & robot = result.robot[team][i];
           for(int i = 0;i < PARAM::ROBOTNUM; i++){
               ...
               if (GlobalData::instance()->commandMissingFrame[team] < 20 && command.use_dir != 0) {
                   //小数部分               
...
//                   if(!command.use_dir) robot.rotateVel = command.vr;
