...
struct Ball {
    ...
    CGeoPoint best_predict_pos; // best predict pos
    CGeoPoint chip_second_pos; // 第二落球點
    double chip_fly_time; // 飞行时间
    double chip_rest_time;
    CVector3 real_pos;
    CVector3 chip_velocity;
...
struct SingleCamera {
    ...
    SingleCamera(): height(cam_height) {} //set 3800mm as an example（in our lab）
...
class OriginMessage {
  public:
    ...
    bool setRobot(int color, int id, const CGeoPoint& point, double angel) {
        ...
        return robot[color][id].fill(point.x(), point.y(), angel);
    }
