    ...
    while(true) {
        ...
        if (receiveSocket[t].state() == QUdpSocket::BoundState && receiveSocket[t].hasPendingDatagrams()) {
            ...
            for(int i = 0; i < commands.command_size(); i++) {
                ...
                if(!isSimulation){
                    globalCommand.robotSpeed[command.robot_id()] = rs;
                    globalCommand.valid = true;
                }
            ...
            if(isSimulation) {
                ...
                if(globalCommand.valid && USEDIR) {
                    GlobalData::instance()->robotCommand[t].push(globalCommand);
//                    qDebug()<<globalCommand.robotSpeed[6].vr;
//                    qDebug()<<GlobalData::instance()->robotCommand[t][0].robotSpeed[6].vr;
                }
