    ...
    if (isRun) {
        ...
        for(int color = PARAM::BLUE; color <= PARAM::YELLOW; color++) {
            ...
            for(int j = 0; j < PARAM::ROBOTNUM; j++) {
                ...
                robot4Rec->set_imudir(GlobalData::instance()->robotInformation[color][j].imuDir);
                robot4Rec->set_imucleaned(GlobalData::instance()->robotInformation[color][j].imucleaned);
                robot4Rec->set_battery(GlobalData::instance()->robotInformation[color][j].battery);
                robot4Rec->set_capacitance(GlobalData::instance()->robotInformation[color][j].capacitance);
                robot4Rec->set_infrared(GlobalData::instance()->robotInformation[color][j].infrared);
            }
        }
        ...
        debugMsgs = recMsg.mutable_athenadebugmsgs();
        debugMsgs->ParseFromArray(GlobalData::instance()->athenaDebugMessage.data(), GlobalData::instance()->athenaDebugMessage.size());
