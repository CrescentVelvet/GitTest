...
void CSimulator::controlRobot(int num,bool team){
    ...
    bool number_control = true;
    ZSS::ZParamManager::instance()->loadParam(number_control,"Sim/use_from_large",true);
    auto* replacement = packet.mutable_replacement();
    for(int i=PARAM::ROBOTNUM-1;i>PARAM::ROBOTNUM-num-1;i--){
        auto* robot = replacement->add_robots();
        robot->set_x(ratio*(PARAM::ROBOTNUM - i)*0.3);
        robot->set_y(0);//(yIn[i]);
        robot->set_id(number_control ? i : PARAM::ROBOTNUM - 1 - i);
        robot->set_dir(dir);
        robot->set_yellowteam(team);
