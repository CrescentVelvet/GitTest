...
void Interaction::setVision(bool needStart, bool real) {
    ...
    if (needStart) {
        ...
        if (real) {
//            RefereeThread::instance()->start();
            GlobalData::instance()->setImuData();
        }
...
bool Interaction::controlMedusa(bool control) {
    if(control == false) {
        if (medusaProcess != nullptr) {
            ...
            medusaProcess = nullptr;
            QTextStream(stdout) << "\n------------------------------------\nexit Medusa\n------------------------------------\n";
            GlobalData::instance()->MedusaOn = false;
        }
        ...
        QTextStream(stdout) << "\n------------------------------------\n" << "running " << name << "\n------------------------------------\n";
        GlobalData::instance()->MedusaOn = true;
    }
    return true;
}
...
void Interaction::imuclean(int team) {
    bool IS_SIM;
    bool USEDIR;
    ...
    //qDebug()<<"imu clean"<<IS_SIM<<PARAM::USE_DIR;
    if(!IS_SIM && USEDIR) {
        GlobalData::instance()->imureset(team);
        GlobalData::instance()->imuend = false;
        if(GlobalData::instance()->imucleaning[team]){
            qDebug()<<"last IMU clean thread is still on";
            return;
        }
        receiveThread = new std::thread([ = ] {GlobalData::instance()->imuClean(team);});
        receiveThread->detach();
    }
}
void Interaction::imucleanend(int team) {
    GlobalData::instance()->imuend = true;
    if(GlobalData::instance()->imucleaning[team]) receiveThread->~thread();
    GlobalData::instance()->imureset(team);
}
...
void Interaction::kill() {
    ...
    ZSS::ZParamManager::instance()->loadParam(USEDIR, "IMU/useDir", true);
    if(USEDIR){
        GlobalData::instance()->saveImuData();
    }
