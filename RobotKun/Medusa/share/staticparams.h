...
namespace PARAM {
    ...
    const int BLUE = 0;
    const int YELLOW = 1;
    const int BALLMERGEDISTANCE = 200;
    const int ROBOTMERGEDOSTANCE = 200;
    const int TEAMS = 2;
    ...
    namespace Field{
        ...
        const double PITCH_MARGIN = 300;                   // 边界宽度
        ...
        const double PENALTY_MARK_X = PITCH_LENGTH / 2 - PENALTY_AREA_DEPTH;               // 点球点的X坐标
    ...
    namespace Vehicle{
        namespace V2{
            const double PLAYER_MOUTH_WIDTH = 78;
            const double PLAYER_SIZE = 90;
            const double PLAYER_FRONT_TO_CENTER = 80.0;
            const double PLAYER_CENTER_TO_BALL_CENTER = 98;
            ...
            const double PLAYER_HEIGHT = 144; // the height of player
        }
    }
    namespace AvoidDist{
        //避障所用参数
...
namespace ZSS {
    ...
    const QString KICKPARAM_ADDRESS = "224.5.23.3";
