...
bool RunPos::isInRangeOfPos(CGeoPoint& _pos1, CGeoPoint& _pos2, float threshold){
    if(_pos1.dist(_pos2) < threshold) return true;
    else return false;
}

bool RunPos::isInStripZone(CGeoPoint& testPos, CGeoPoint& _pos1, CGeoPoint& _pos2, float threshold){
...
std::vector<CGeoPoint> RunPos::sampling(int x, int y, int threshold_x, int threshold_y){
    std::vector<CGeoPoint> sample_list;
    int number = 1000;
    for(int i=0; i<number; i++){
        int rand_x = rand()%threshold_x + x + 1;
        int rand_y = rand()%(2 * y) - y;
        CGeoPoint rand_point(rand_x, rand_y);
        sample_list.push_back(rand_point);
    }

    return sample_list;
}

void RunPos::saoAction(const CVisionModule* pVision){
    int threshold_x = 400;
    int threshold_y = 300;
    int buffer = 500;
    std::vector<int> defender_list;
    CGeoPoint target_pos(0,0);
...
//    for(std::vector<int>::iterator iter = defender_list.begin(); iter != defender_list.end(); ++iter){
//        target_pos.fill(target_pos.x() + pVision->theirPlayer(*iter).Pos().x(), target_pos.y() + pVision->theirPlayer(*iter).Pos().y());
//    }

//    target_pos.fill(target_pos.x()/defender_list.size(), target_pos.y()/defender_list.size());
    cout<<"size of defender_list:"<<defender_list.size()<<endl;
//    cout<<"x:"<<target_pos.x()<<"  y:"<<target_pos.y()<<endl;
//    _runPos[1].pos = target_pos;

    std::vector<CGeoPoint> search_list = sampling(PARAM::Field::PITCH_LENGTH/2 - PARAM::Field::PENALTY_AREA_DEPTH - buffer, PARAM::Field::PENALTY_AREA_WIDTH / 2 + buffer, threshold_x, threshold_y);
    int min_dist = 10000;
    CGeoPoint min_point;
    for(int i=0; i < search_list.size(); i++){
        int dist = 0;
        for(int j=0; j < defender_list.size(); j++){
            dist += search_list[i].dist(pVision->theirPlayer(defender_list[j]).Pos());
        }
        if(dist < min_dist){
            min_dist = dist;
            min_point = search_list[i];
        }
    }
    cout<<"min_point:"<<min_point.x()<<"   "<<min_point.y()<<endl;
    _runPos[1].pos = min_point;

    return;

}

void RunPos::generateRunPos(const CVisionModule* pVision, const CGeoPoint& avoidPos, const CGeoPoint& avoidPos2) {
    // 更新图像信息
    _pVision = pVision;
    _avoidPos = avoidPos;
    ...
    for (int i = 0; i < ATTACK_AREA_NUM; i++) {
        ...
        if (Utils::IsInField(_avoidPos) || Utils::IsInField(_avoidPos2)) {
            if (ATTACK_AREA[i].HasPoint(_avoidPos) || (ATTACK_AREA[i].HasPoint(_avoidPos2) && _pos2Switch)) {
...
void RunPos::selectAreaBestPoint(int areaNum) {
    ...
    for (float bx = ATTACK_AREA[areaNum]._point[0].x() + 1; bx < ATTACK_AREA[areaNum]._point[2].x(); bx += 100) {
        for (float by = ATTACK_AREA[areaNum]._point[0].y() + 1; by < ATTACK_AREA[areaNum]._point[2].y(); by += 100) {
...
//            qDebug() << "areaNum:" << j << "ourCarNum:" << num;
            if (ATTACK_AREA[j].HasPoint(_runPos[i].pos) && /*num <= 1 &&*/ !ATTACK_AREA[j].HasPoint(_avoidPos) && (!(ATTACK_AREA[j].HasPoint(_avoidPos2) && _pos2Switch ))) {
    ...
    for (int m = 0; m < 2; m++) {
        if (_runPos[m].isValid) {
           ...
           // if(m == 1)
//           GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(-PARAM::Field::PITCH_LENGTH/2+200, PARAM::Field::PITCH_WIDTH/2-1800) , QString("new pos!!!").toLatin1(), COLOR_ORANGE);
...
//                if(m == 1) GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(-PARAM::Field::PITCH_LENGTH/2+200, PARAM::Field::PITCH_WIDTH/2-2000) , QString("new pos2!!!").toLatin1(), COLOR_ORANGE);
//                if(m == 1 && areaRawPoint[n].pos.x() == 9999) GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(-PARAM::Field::PITCH_LENGTH/2+200, PARAM::Field::PITCH_WIDTH/2-2200) , QString("99999   %1  %2 %3 %4 %5!!!").arg(n).arg(ATTACK_AREA[n]._point[0].x()).arg(ATTACK_AREA[n]._point[0].y()).arg(ATTACK_AREA[n]._point[1].x()).arg(ATTACK_AREA[n]._point[1].y()).toLatin1(), COLOR_ORANGE);
