...
double CCompensate::checkCompensate(double ballspeed,double rawdir){
    ...
    int  row =ceil((ballspeed-1950)/50);
	if (column > 17)
        column = 17;
	if (row<1){
		row = 1;
    ...
    double distup = ballspeed - ((row -1)*50+1950);
    double distdown = row*50+1950 - ballspeed;
	double leftfactor = distright/5;
	double rightfactor = distleft/5;
    double upfactor = distdown/50;
    double downfactor = distup/50;
