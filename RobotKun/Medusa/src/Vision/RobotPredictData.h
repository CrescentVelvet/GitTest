...
struct RobotCommandEffect{
    RobotCommandEffect():cycle(0),rot_speed(0),use_dir(false){}
...
public:
    ...
    void setCommand(int cycle, const CVector& vel, double rot_speed, bool use_dir)
	{
        ...
        if ( std::isfinite(vel.mod()) && std::isfinite(rot_speed) ){
            ...
            _command[(cycle+MAX_INFORMATIONS) % MAX_INFORMATIONS].use_dir = use_dir;
		}
		else{
            ...
            _command[(cycle+MAX_INFORMATIONS) % MAX_INFORMATIONS].use_dir = false;
		}
	}
