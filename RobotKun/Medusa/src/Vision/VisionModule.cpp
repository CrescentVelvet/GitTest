...
//    bool VERBOSE_MODE = false; // 输出信息
    ...
    bool isYellow = false;
    bool side = false;
    const int TEAM_BINARY[2] = {(1 << 8),(1 << 9)};
...
void CVisionModule::registerOption(const COptionModule* pOption) {
    _pOption = pOption;
    _gameState.init(TEAM_BINARY[pOption->MyColor()]);
}
        ...
        while (visionSocket->state() == QUdpSocket::BoundState && visionSocket->hasPendingDatagrams()) {
            ...
            if (ballFound) {
                ...
                if (ball.x() > 88888 && ball.y() > 88888) {
                    ...
                    //chip option info
                    if(ball.has_real_x()&&ball.has_real_y()&&ball.has_real_z())
                        visionTemp.realPos.formXYZ(ball.real_x(), ball.real_y(), ball.real_z());
                    else
                        visionTemp.realPos.formXYZ(99999, 99999, 99999);
                    if(ball.has_chip_vel_x()&&ball.has_chip_vel_y()&&ball.has_chip_vel_z())
                        visionTemp.chipVel.formXYZ(ball.chip_vel_x(), ball.chip_vel_y(), ball.chip_vel_z());
                    else
                        visionTemp.chipVel.formXYZ(99999, 99999, 99999);
                    if(ball.has_chip_predict_x()&&ball.has_chip_predict_y()){
                        visionTemp.chipPredict.x = ball.chip_predict_x();
                        visionTemp.chipPredict.y = ball.chip_predict_y();
                    }
                    else{
                        visionTemp.chipPredict.x = 99999;
                        visionTemp.chipPredict.y = 99999;
                    }
                    if(ball.has_chip_best_predict_x()&&ball.has_chip_best_predict_y()){
                        visionTemp.bestChipPredict.x = ball.chip_best_predict_x();
                        visionTemp.bestChipPredict.y = ball.chip_best_predict_y();
                    }
                    else{
                        visionTemp.bestChipPredict.x = 99999;
                        visionTemp.bestChipPredict.y = 99999;
                    }
                    if(ball.has_chip_second_pos_x()&&ball.has_chip_second_pos_y()){
                        visionTemp.secondChipPos.x = ball.chip_second_pos_x();
                        visionTemp.secondChipPos.y = ball.chip_second_pos_y();
                    }
                    else{
                        visionTemp.secondChipPos.x = 99999;
                        visionTemp.secondChipPos.y = 99999;
                    }
                    if(ball.has_chip_fly_time())
                        visionTemp.chipFlyTime = ball.chip_fly_time();
                    else
                        visionTemp.chipFlyTime = 99999;
                    if(ball.has_chip_rest_fly_time())
                        visionTemp.chipRestFlyTime = ball.chip_rest_fly_time();
                    else
                        visionTemp.chipRestFlyTime = 99999;
                }
            } else {
            ...
            for (int color = 0; color < PARAM::TEAMS; color++) {
                ...
                for (int i = 0; i < PARAM::ROBOTNUM; i++) {
                    ...
                    if(side) visionTemp.player[color][index].imu_dir = robot.imu_dir() + PARAM::Math::PI;
                    else visionTemp.player[color][index].imu_dir = robot.imu_dir();
                    visionTemp.player[color][index].imu_rotate_vel = robot.imu_rotate_vel();
                }
            }
...
void CVisionModule::setNewVision() {
    ...
    BallStatus::Instance()->setChipKickState(_info.BallState == _chip_pass);
...
//    ballPosCollectorCounter = (ballPosCollectorCounter + 1) % 10;
    ...
    if ((_info.BallState == _chip_pass || BallStatus::Instance()->IsChipKickedOut()) && _rawBallPos.Vel().mod() > 50) {
...
void CVisionModule::updateNextRefereeMsg(const unsigned int command){
    ...
    if(command < 2){
        ...
        if(Command2Message[int(command/2)]==""){
            ...
            _nextRefereeMsg = (((_pOption->MyColor() == PARAM::YELLOW ?0:1)==command%2)?"Our":"Their")+Command2Message[int(command/2)];
        }
    }else{
        _nextRefereeMsg = "";
    }
