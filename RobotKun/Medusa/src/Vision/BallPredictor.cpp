...
void CBallPredictor::updateVision( const CServerInterface::VisualInfo& vInfo, bool invert) {
    ...
    thisCycle.SetRealPos(vInfo.realPos.x()*invertFactor,vInfo.realPos.y()*invertFactor,vInfo.realPos.z());
    thisCycle.SetChipKickVel(vInfo.chipVel.x()* invertFactor,vInfo.chipVel.y()* invertFactor,vInfo.chipVel.z());
    thisCycle.SetChipPredict(vInfo.chipPredict.x*invertFactor, vInfo.chipPredict.y*invertFactor);
    thisCycle.SetBestChipPredictPos(vInfo.bestChipPredict.x*invertFactor, vInfo.bestChipPredict.y*invertFactor);
    thisCycle.SetSecondChipPos(vInfo.secondChipPos.x*invertFactor, vInfo.secondChipPos.y*invertFactor);
    thisCycle.SetChipBallState(vInfo.BallState);
    thisCycle.SetChipFlyTime(vInfo.chipFlyTime);
    thisCycle.SetChipRestFlyTime(vInfo.chipRestFlyTime);
    thisCycle.SetSecondChipPos(vInfo.secondChipPos.x*invertFactor, vInfo.secondChipPos.y*invertFactor);
