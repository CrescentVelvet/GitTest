...
CRobotPredictor::CRobotPredictor() : _robotLostTime(0), _isHasRotation(true) {
    ZSS::ZParamManager::instance()->loadParam(USE_DIR, "IMU/useDir", false);
...
void CRobotPredictor::updateVision(int cycle, const VehicleInfoT& player, const MobileVisionT& ball, bool invert) {
    ...
    predictedVision.SetImuDir(player.imu_dir);
    predictedVision.SetImuRotateVel(player.imu_rotate_vel);
    ...
    if (_isHasRotation) {//our robot
        ...
        // 这个地方应引起注意!!!!!!
        // modified by chenzhike 2020.11.1 用了imu之后把转速设置为转角所以不能简单赋值 需要改动command设置下发，所以先把图像的转速设置为状态, 改动：下发command需要知道是否use_dir
        if(USE_DIR && cmd.use_dir ) {
            predictedVision.SetRotVel(dirVel);
        }
        else predictedVision.SetRotVel(cmd.rot_speed); // 线性滤波
//        predictedVision.SetRotVel(rawDirVel);
    } else {
...
void CRobotPredictor::updateCommand(int cycle, const CPlayerCommand * cmd) {
    _commandLogger.setCommand(cycle, cmd->getAffectedVel(), cmd->getAffectedRotateSpeed(), cmd->getAffectedUseDir());
}
