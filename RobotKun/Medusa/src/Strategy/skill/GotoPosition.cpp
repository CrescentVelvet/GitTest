...
namespace{
    ...
    /// 用于解决到点晃动的问题
    const double DIST_REACH_CRITICAL = 70;	// [unit : mm]
...
CPlayerCommand* CGotoPositionV2::execute(const CVisionModule* pVision)
{
    ...
    /// 设定目标状态
    ///
    bool use_dir = task().player.use_dir;
    PlayerPoseT final;
    final.SetPos(target);
    ...
    if((playerFlag&PlayerStatus::RUSH) && (abs(Utils::Normalize(self.Dir() - final.Dir())) < PARAM::Math::PI/180 *5  ||RobotSensor::Instance()->IsInfraredOn(vecNumber))) {
        //GET BALL RUSH 状态先转动再加速
        use_dir = false;
        GDebugEngine::Instance()->gui_debug_msg(self.Pos()+CVector(100,-100), QString("rushrushrush").toLatin1());
        GDebugEngine::Instance()->gui_debug_msg(self.Pos()+CVector(100,-200), QString("final velocity: %1").arg(task().player.vel.mod()).toLatin1());
    }
    ...
    if(DRAW_TARGET){
        ...
        GDebugEngine::Instance()->gui_debug_line(final.Pos(), final.Pos() + Utils::Polar2Vector(200, final.Dir()), 10, 255255);
        GDebugEngine::Instance()->gui_debug_line(self.Pos(), self.Pos() + Utils::Polar2Vector(200, final.Dir()), 10, 255255);
    }
    /// 调用控制方法
    ...
    /// 进行轨迹生成并记录理想执行时间
    control.makeCmTrajectory(self, final, capability, mode, use_dir);					// CMU 非零速到点
...
//            GDebugEngine::Instance()->gui_debug_msg(self.Pos(), QString("%1").arg(task().player.max_acceleration).toLatin1(),COLOR_BLUE);
    ...
    if((playerFlag&PlayerStatus::RUSH) && abs(Utils::Normalize(self.Dir() - final.Dir())) > PARAM::Math::PI/180 *20) {
        //GET BALL RUSH 状态先转动再加速
      localVel = localVel*0.95;
      GDebugEngine::Instance()->gui_debug_msg(self.Pos()+CVector(100,0), QString("slowdown").toLatin1());
    }
    if(DRAW_DEBUG_MSG) {
        ...
        GDebugEngine::Instance()->gui_debug_msg(self.Pos()+ CVector(100, 100), QString("globalVel: (%1, %2) maxspeed: %3").arg(globalVel.x()).arg(globalVel.y()).arg(capability.maxSpeed).toLatin1());
        GDebugEngine::Instance()->gui_debug_msg(self.Pos()+ CVector(100, 200), QString("targetPos: (%1, %2) maxspeed: %3").arg(target.x()).arg(target.y()).arg(capability.maxSpeed).toLatin1());
        GDebugEngine::Instance()->gui_debug_msg(self.Pos(), QString("localvel Dir: (%1, %2)  %3").arg(localVel.x()).arg(localVel.y()).arg(self.Dir()).toLatin1());
    }
...
//    GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(0,0), QString("%1, %2").arg(task().player.use_dir).arg(task().player.use_dir).toLatin1(),COLOR_BLUE);
    double commandangle = use_dir? Utils::Normalize(task().player.angle) : rotVel;
    /// 生成并返回控制指令
...
//        return pCmdFactory->newCommand(CPlayerSpeedV2(vecNumber, 0, 0, rotVel, dribblePower));
        return pCmdFactory->newCommand(CPlayerSpeedV2(vecNumber,
                                                      localVel.x(),
                                                      localVel.y(),
                                                      commandangle/*Utils::Normalize(2 - task().player.dirBias)*/,
                                                      use_dir,
                                                      dribblePower));
}
