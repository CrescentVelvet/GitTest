...
CZAttack::CZAttack() : fraredOn(0) {
    ...
    ZSS::ZParamManager::instance()->loadParam(IGNORE_CLOSE_ENEMY_DIST,"Messi/IGNORE_CLOSE_ENEMY_DIST",700);
}
...
//    bool enemyDribble = enemy2ball.mod() < PARAM::Vehicle::V2::PLAYER_SIZE + 5.0;
    ...
    CGeoPoint shoot_target(PARAM::Field::PITCH_LENGTH/2,0);
    bool canShootGoal = ShootModule::Instance()->generateBestTarget(pVision,shoot_target,ball.Pos());
...
//    bool use_zpass = (me.X() > 350*10 && fabs(me.Y()) < 220*10) && !canShootGoal;
    if (ourTime + deltaTime < theirTime || fraredOn > 10){
        ...
        if (shootGoal){
            // can shoot goal
            bool inTheirPenalty = Utils::InTheirPenaltyArea(me.Pos(), 1000);
            if (canShootGoal || !inTheirPenalty){
                //zget
...
//                }
        ...
        // pass ball
            ...
            // zget
            GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(100,200), QString("theirTime: %1").arg(theirTime).toLatin1());
        ...
        if(taskMode == TaskMode::ZGET && !Utils::InTheirPenaltyArea(me.Pos(), 15*PARAM::Vehicle::V2::PLAYER_SIZE)){
            ...
            if (WorldModel::Instance()->getEnemyAmountInArea(me.Pos(), minBreakDist, enemyNumVec, 0)) {
                for (auto enemyNum : enemyNumVec) {
                    ...
                    bool enemyInFront = fabs(Utils::Normalize((pVision->theirPlayer(enemyNum).Pos() - me.Pos()).dir() - me.Dir())) < PARAM::Math::PI/3;
                    bool enemyTooClose = pVision->theirPlayer(enemyNum).Pos().dist(me.Pos()) <= IGNORE_CLOSE_ENEMY_DIST;
                    if (enemyInFront && enemyTooClose){
                        if (verbose) GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(100,100), QString("ZGET -> ZBREAK").toLatin1());
                        taskMode = TaskMode::ZBREAK;
                        break;
                    }
                }
...
//        }
//        if(enemyHoldBall && enemyFaceGoal && me2ball.mod() - enemy2ball.mod() > 2 * PARAM::Vehicle::V2::PLAYER_SIZE){
