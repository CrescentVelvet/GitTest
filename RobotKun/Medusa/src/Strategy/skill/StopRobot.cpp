...
CPlayerCommand* CStopRobotV2::execute(const CVisionModule* pVision)
{
    ...
    if( vecSpeed > decSpeedPerFrame ){
        ...
        return CmdFactory::Instance()->newCommand(CPlayerSpeedV2(task().executor, newVel.x(), newVel.y(), task().player.use_dir,3, DRIBBLE_DISABLED));
    }
    else
//        return CmdFactory::Instance()->newCommand(CPlayerSpeedV2(task().executor, 0, 0, 0, DRIBBLE_DISABLED));
        return CmdFactory::Instance()->newCommand(CPlayerSpeedV2(task().executor, 0, 0, 0, false, DRIBBLE_DISABLED));
} 
