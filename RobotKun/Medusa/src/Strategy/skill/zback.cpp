...
void CZBack::plan(const CVisionModule *pVision)
{
    ...
    if(defendNum < 0){
        ...
        if (missingGuard > 0 && GuardPos::Instance()->validBackPos(guardPos, player)){
            ...
            guardPos = GuardPos::Instance()->backPos(validGuard, newindex, player,-1);
        }
    ...
    if (DEBUG_ZBACK){
        ...
        GDebugEngine::Instance()->gui_debug_msg(guardPos, QString("  %1 %2 defendNum: %3").arg(player).arg(index).arg(defendNum).toLatin1(), COLOR_CYAN);
    }
