...
namespace {
	...
	//>需要用到的常量
    const double newVehicleBuffer = 0.6*10;               // 小嘴巴机器人PLAYER_FRONT_TO_CENTER补偿
	const double ballPredictBaseTime = 0.2;            // 球预测的基础时间
	const double SpeedLimitForPredictTime = 120;       // 球在该速度以上则加大预测时间
    const double nearBallRadius = 12*10;                  // 小车半径+球半径+2~3视频误差裕度,判定是否需要AVOIDBALL的半径
    const double ball2myheadLimit = 3*10;                 // 小嘴巴车嘴长7cm，球半径2cm，想要稳定拿住球需要(7-2)/2=2.5cm 再加1.0的余值
    const double directGetBallDist = 20*10;               // 直接冲上去拿球的距离
    const double avoidBallSuccessDist = 25*10;            // 判定避球成功的距离 25
    const double minGetBallDist = 17*10;
    const double maxGetBallDist = 28*10;				   // 25
    const double AllowFaceToFinalDist = 200*10;           // 进入该距离后开始转向
    const double transverseBallSpeed = 20*10;             // 对拿球产生影响的最低横向球速 ori：30
