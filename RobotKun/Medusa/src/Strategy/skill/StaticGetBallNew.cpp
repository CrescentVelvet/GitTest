...
namespace {
    bool Verbose = true;
    const double newVehicleBuffer = 0.6 * 10;
    const double directGetBallDist = 35 * 10;
    const double StopDist = 12 * 10;
...
void CStaticGetBallNew::plan(const CVisionModule * pVision)
{
    ...
    double finalAngel = task().player.angle;
    ...
    if (!canGetBall && me2Ball.mod() < 30 * 10 ) needAvoidBall = true;
	else needAvoidBall = false;
	
    if (Verbose) GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(160 * 10, -150 * 10), "STATIC", COLOR_CYAN);
	static int staticDir;
	staticDir = getStaticDir(pVision, staticDir);
	if (needAvoidBall) {
        if (Verbose) GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(120 * 10, -150 * 10), "AVOID BALL", COLOR_CYAN);
		if (fabs(me2BallDirDiff) > PARAM::Math::PI / 3) {
            if (Verbose) GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(80 * 10, -150 * 10), "beside ball", COLOR_CYAN);
			double avoidDir = Utils::Normalize(ball2Me.dir() + staticDir * PARAM::Math::PI / 4);
            static_get_task.player.pos = ball.Pos() + Utils::Polar2Vector(30 * 10, avoidDir);
            finalAngel = me2Ball.dir();
		}
		else {
            if (Verbose) GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(80 * 10, -150 * 10), "behind ball", COLOR_CYAN);
            double directDist = PARAM::Vehicle::V2::PLAYER_FRONT_TO_CENTER + newVehicleBuffer + PARAM::Field::BALL_SIZE + StopDist - 2.5 * 10;
        ...
        if (Verbose) GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(120 * 10, -150 * 10), "GO TO", COLOR_CYAN);
		if (fabs(me2BallDirDiff) > PARAM::Math::PI / 2) {
            if (Verbose) GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(80 * 10, -150 * 10), "in front of ball", COLOR_CYAN);
			double gotoDir = Utils::Normalize(finalAngel + staticDir * PARAM::Math::PI * 3 / 5);
            static_get_task.player.pos = ball.Pos() + Utils::Polar2Vector(30 * 10, gotoDir);
            static_get_task.player.vel = Utils::Polar2Vector(100 * 10, (static_get_task.player.pos - me.Pos()).dir());
		}
		else {
            if (Verbose) GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(80 * 10, -150 * 10), "behind ball", COLOR_CYAN);
            double directDist = PARAM::Vehicle::V2::PLAYER_FRONT_TO_CENTER + newVehicleBuffer + PARAM::Field::BALL_SIZE + StopDist - 2.5 * 10;
