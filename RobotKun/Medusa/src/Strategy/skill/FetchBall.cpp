...
void CFetchBall::plan(const CVisionModule* pVision) {
    ...
    /*********************** set subTask ********************/
    if(ball.Valid() && !Utils::IsInField(ball.RawPos(), PARAM::Vehicle::V2::PLAYER_SIZE)){//ball out of field
        if(fraredOn < 2*BPFraredOn){//not have ball
            ...
            setSubTask(PlayerRole::makeItGoto(vecNumber, Utils::MakeInField(me.RawPos(), 2*PARAM::Vehicle::V2::PLAYER_SIZE), me2target.dir(), CVector(0, 0), 0, 3000, 5, 3000, 5, PlayerStatus::ALLOW_DSS|PlayerStatus::NOT_AVOID_PENALTY));
        }
    }
    else{
        if(fraredOn < 2*BPFraredOn && cnt == 0 && !lastInfered){//not have ball
           // GDebugEngine::Instance()->gui_debug_msg(me.Pos()+ Utils::Polar2Vector(2*DEBUG_TEXT_HIGH, -PARAM::Math::PI/1.5), QString("state:%1, chip:%2").arg(fraredOn).arg(cnt).toLatin1(), COLOR_WHITE);
            ...
            if(ball.Vel().mod()> 300 || ball.Pos().dist(me.Pos()) > 700){
                kick_flag = kick_flag | PlayerStatus::NOT_AVOID_PENALTY | PlayerStatus::DRIBBLE;
                setSubTask(PlayerRole::makeItGetBallV4(vecNumber, kick_flag, target,CGeoPoint(99999,99999),0));
//                GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(0,0),"FETCHBALL_GETBALL!!!!!",1,0,500);
            }else{

                setSubTask(PlayerRole::makeItGoto(vecNumber, ball.RawPos(), me2ball.dir(), PlayerStatus::ALLOW_DSS|PlayerStatus::NOT_AVOID_PENALTY|PlayerStatus::DRIBBLING));
            }
        }
        else{//have ball
            if(!SAFEMODE){
...
//                    }
                    ...
                    setSubTask(PlayerRole::makeItGoto(vecNumber, target + Utils::Polar2Vector(-PARAM::Vehicle::V2::PLAYER_CENTER_TO_BALL_CENTER, me2target.dir()), me2target.dir(), CVector(0, 0), 0, 2000, 20, 2000, 10, PlayerStatus::ALLOW_DSS|PlayerStatus::NOT_AVOID_PENALTY|PlayerStatus::DRIBBLING));
                }
            }
        }
    }
...
//    if (cnt < 30 && (me2ball.mod() < 50 || frared)) {
    ...
    /******************************/
    lastInfered = RobotSensor::Instance()->IsInfraredOn(vecNumber);
