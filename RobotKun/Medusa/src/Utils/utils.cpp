...
//        GDebugEngine::Instance()->gui_debug_x(pos + Polar2Vector(pow(vel.mod(), 2) / (2 * 400), vel.dir()));
    ...
    CGeoPoint MakeOutOfCircleAndOutOfPenalty(const CGeoPoint center, const double radius, const CGeoPoint p, const double buffer)
    {
        ...
        CGeoRectangle fieldRect(CGeoPoint(-PARAM::Field::PITCH_LENGTH / 2, PARAM::Field::PENALTY_AREA_WIDTH / 2 + buffer), CGeoPoint(-PARAM::Field::PITCH_LENGTH / 2 + PARAM::Field::PENALTY_AREA_DEPTH + buffer, -PARAM::Field::PENALTY_AREA_WIDTH / 2 - buffer));
        CGeoPoint newPos(center + p2c * (radius + buffer) / dist);
        if(fieldRect.HasPoint(p)){
	...
	CGeoPoint MakeOutOfCircleAndInField(const CGeoPoint& center,const double radius,const CGeoPoint& p,const double buffer)
	{
        ...
        GDebugEngine::Instance()->gui_debug_x(newPos,COLOR_BLACK);
