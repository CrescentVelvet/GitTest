...
//    static bool leftFirst = false;
    ...
    getDefenceTargetAndLine(_RdefendTarget,_RdefendDir);
...
// 　　球看不到就跟车
    if(!pVision->ball().Valid()){
        RballPos = reversePoint(pVision->theirPlayer(ZSkillUtils::instance()->getTheirBestPlayer()).Pos());
        RBallPosX = -1 * pVision->theirPlayer(ZSkillUtils::instance()->getTheirBestPlayer()).Pos().x();
    }
    const MobileVisionT& ball = pVision->ball();
    CGeoPoint BallPos = reversePoint(RballPos);
    //    const MobileVisionT& ball = pVision->ball().Valid() ? pVision->ball(): pVision->theirPlayer(ZSkillUtils::instance()->getTheirBestPlayer());
    ...
    if (leftBack != 0 && rightBack != 0) {
        ...
        if (ballShooted == true) {
            if (RBallPosX < 0)
                ballShooted = (ball.Vel().mod() > 2500)? true : false;
            else if (RBallPosX < PITCH_LENGTH/4)
                ballShooted = (ball.Vel().mod() > 1500)? true : false;
            else
                ballShooted = (ball.Vel().mod() > 1000)? true : false;
        }
    ...
    if (ball.Vel().dir() < (LEFT_POST - BallPos).dir() || ball.Vel().dir() > (RIGHT_POST - BallPos).dir()) {
