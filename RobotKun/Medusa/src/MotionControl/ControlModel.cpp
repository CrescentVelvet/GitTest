...
void CControlModel::makeCmTrajectory(const PlayerPoseT& start, const PlayerPoseT& final, const PlayerCapabilityT& capability, nonZeroMode mode, bool use_dir)
{
    _pathList.clear();
    ...
    if(IS_SIMULATION) {
        accel_factor = 1.5;
        angle_accel_factor = 2.0;
    }

//    cout << "ControlModel"<< endl;
//    cout << start.X()<<" "<<start.Y()<<" "<<start.Vel().mod()<<" "<<start.RotVel()<< endl;
//    cout << final.X() << " " << final.Y() << " " << final.Vel().mod() << endl;
    goto_point_omni(start,final,capability,accel_factor,angle_accel_factor,_nextStep, mode, use_dir);
}
