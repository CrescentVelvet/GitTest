...
const double min_max_angle_speed = 6.0;
...
void compute_motion_1d(double x0, double v0, double v1,
    ...
    double period; // 一段很小的时间，处理运动到目标点附近时加速度，稳定到点，防止超调
    if(pT == MOVE_X)
        period = 1 / 50.0;
    else if(pT == MOVE_Y)
        period = 1.0 / 80.0;
        ...
        if (fabs(v1) > 1e-8 && mode == ACCURATE) {
            ...
            if (time_to_stop < period) {
                if(pT == MOVE_Y)
                    ...
                    //qDebug()<<"time to stop"<<time_to_stop;
...
//        v_max = v1.mod();
    ...
    if(time_x < 1e-5 || time_x > 500) time_x = 0;
    if(time_y < 1e-5 || time_y > 500) time_y = 0;
    if(time_x < time_y) {
...
void goto_point_omni( const PlayerPoseT& start,
    ...
    //GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(0,0),QString("rotvel:%1, rotacc:%2, rotdec:%3").arg(max_angle_speed).arg(max_angle_accel).arg(max_angle_decel).toLatin1());
    CVector a;
...
//    rotateFactor = 1.0;
    ...
    if(DISPLAY_ROTATION_LIMIT){
        ...
        GDebugEngine::Instance()->gui_debug_msg(target_pos+CVector(0,-180*10), QString("maxRotateDcc:   %1").arg(max_angle_decel).toLatin1());
        ...
        GDebugEngine::Instance()->gui_debug_msg(target_pos+CVector(0,-100*10), QString("start angle:  %1").arg(start.Dir()).toLatin1());
        GDebugEngine::Instance()->gui_debug_msg(target_pos+CVector(0,-120*10), QString("target angle:  %1").arg(final.Dir()).toLatin1());
        GDebugEngine::Instance()->gui_debug_msg(target_pos+CVector(0,-140*10), QString("delta angle:  %1").arg(ang).toLatin1());
    }
