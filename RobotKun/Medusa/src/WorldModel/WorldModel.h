...
public:
    ...
    void setBPFinish(const bool finish) { bpFinish = finish; }
    bool getBPFinish() {
        bool res = bpFinish;
        bpFinish = false;
        return res;
...
//	}
    ...
    double getTandemDir(int myNum=myDefaultNum){
		CGeoPoint tandemPos=CGeoPoint(0,0);
    ...
    double regulateCheck(int player, double needBallVel, double needDir, bool isChip, double tolerance);
...
//    bool isMarked(int);
    ...
    bool getEnemyAmountInArea(const CGeoPoint &center , double radius, double buffer = 0);
    bool getOurAmountInArea(double x1, double x2, double y1, double y2, double buffer = 0);
    bool getOurAmountInArea(const CGeoPoint& center ,double radius, double buffer = 0);
    ...
    int getOurAmountInArea(double x1, double x2, double y1, double y2, std::vector<int>& ourNumVec, double buffer = 0);
    int getOurAmountInArea(const CGeoPoint& center ,double radius, std::vector<int>& ourNumVec, double buffer = 0);
    double preditTheirGuard(const PlayerVisionT& enemy, CGeoPoint& p);
    double preditOurGuard(const PlayerVisionT& enemy, CGeoPoint& p);
